...
pipeline {
  ...
  environment {
    ...
    RELEASE_NAME = "corejava15withmaven"
    ...
    JAVA_MVN_IMAGE_VERSION = "maven:3.8.1-openjdk-17-slim" //https://hub.docker.com/_/maven/tags 
                  ...
                  stage('SonarQube') {
                   ...
                   stage('Build') {
                    echo "echoed BUILD_TAG--- $BUILD_TAG"
                     sh """
                        docker run --rm -v "$WORKSPACE":/usr/src/mymaven -w /usr/src/mymaven $JAVA_MVN_IMAGE_VERSION mvn install -Dmaven.test.skip=true
